...
void llvm_set_out(std::string file) { output_file = file; }
...
// Insert instructions to update cur_context to the correct context before a
// call, and restore it after the call transitions maps the current context to
// what the context should be changed to for the call
void deconstruct_context_transition(llvm::CallInst *call, llvm::GlobalVariable *cur_context,
                                    const std::map<int, int> &transitions) {
  ...
  if (transitions.size() == 1) {
    // Set the new context
    new llvm::StoreInst(llvm::ConstantInt::get(int_type, transitions.begin()->second), cur_context,
                        call);
    return;
  }
  ...
  // For each possible current context, set cur_context to the context that will
  // be transitioned to
  auto final_transition = --transitions.end();
  for (auto it = transitions.begin(); it != final_transition; ++it) {
    llvm::Value *cond =
        builder.CreateICmpEQ(old_context, llvm::ConstantInt::get(int_type, it->first));
...
// If instructions are created to accomplish this, they are inserted before
// insert_before
llvm::Value *get_value(SSA_Opd *operand, int operand_num, int node_num,
  ...
  case SSA_VarOpd: {
    ...
    int value;
    if (program->get_dfg_propagated_value({{operand->get_opd_var(), meta.first}, meta.second},
                                          &value)) {
...
void create_assignment(std::list<SSA_Stmt *> *stmts, llvm::Instruction *insert_before,
                       ...
                       std::map<std::string, llvm::GlobalVariable *> &qdef_globals,
                       llvm::Value **rhs) {
  CHECK_INVARIANT(stmts->size() > 0,
                  "Expected at least one statement for each metamorphic assignment");
  auto final_stmt = --stmts->end();
  CHECK_INVARIANT((*final_stmt)->get_type() == SSA_AssignStmt,
                  "Expected final stmt to be an assign stmt");

  // Phi nodes are implicitly decomposed, so only the final stmt needs to be
  // handled
...
void deconstruct_metamorphic_assign(std::map<int, SSA_Meta *> *metas, llvm::Instruction *assign,
                                    llvm::GlobalVariable *cur_context,
                                    std::map<std::string, llvm::GlobalVariable *> &qdef_globals) {
  ...
  if (metas->size() == 1) {
    ...
    create_assignment(metas->begin()->second->get_stmts(), assign, defs_return_variable,
                      qdef_globals, &rhs);
  ...
  // For each metamorphic assignment, create the corresponding assignment
  // depending on the current context
  auto final_meta = --metas->end();
  for (auto it = metas->begin(); it != final_meta; ++it) {
    llvm::Value *cond = builder.CreateICmpEQ(
        cur_context_value, llvm::ConstantInt::get(int_type, it->second->get_meta_num().second));
...
void deconstruct_single_partition(std::map<std::string, llvm::GlobalVariable *> &qdef_globals,
                                  const std::string &context_var_name) {
  // Determine which functions actually have metamorphic assignments and thus
  // require context transitions when called
  ...
  llvm::Type *int_type = llvm::IntegerType::get(module->getContext(), 32);
  llvm::GlobalVariable *cur_context =
      new llvm::GlobalVariable(*module, int_type, false, llvm::GlobalValue::InternalLinkage,
    ...
    for (int node : pair.second->get_ssa_nodes()) {
      ...
      if (ssa_node->get_type() == SSA_NodeType::SSA_CallNode) {
        ...
        if (func_uses_context.find(ssa_node->get_callee()) != func_uses_context.end()) {
          deconstruct_context_transition(llvm::dyn_cast<llvm::CallInst>(value), cur_context,
                                         program->get_dfg_transitions(ssa_node->get_node_id()));
        }
        continue;
      }
      ...
      deconstruct_metamorphic_assign(metas, llvm::dyn_cast<llvm::Instruction>(value), cur_context,
                                     qdef_globals);
    }
  }
