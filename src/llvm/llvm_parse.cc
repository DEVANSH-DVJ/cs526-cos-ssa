...
void llvm_set_in(llvm::Module *llvm_module) { module = llvm_module; }
...
bool is_usable_func(llvm::Function *func) { return !func->isDeclaration(); }

// Constructs the CFG for a given funtion
// Returns a map of basic blocks to sets of their successors
std::map<llvm::BasicBlock *, std::set<llvm::BasicBlock *>>
construct_llvm_cfg(llvm::Function *func) {
...
GlobalInfo get_globals(llvm::Module *module) {
  // Track which variables have their address taken; these are not suitable to
  // process
    ...
    for (llvm::Use &use : var.uses()) {
      ...
      if (llvm::LoadInst *load = llvm::dyn_cast<llvm::LoadInst>(user)) {
        if (&var != load->getPointerOperand()) {
          // If the address of this variable is not being loaded from, it is not
          // suitable
          found = true;
          break;
        }
        ...
        if (&var != load->getPointerOperand()) {
          // If the address of this variable is not being stored to, it is not
          // suitable
          found = true;
          break;
        }
      } else {
        // The address of this variable is involved in something that is not a
        // load/store, it is not suitable
        found = true;
        break;
      }
    }
...
// Takes an operand that belongs to something that will become a CFG node
// If this operand is used by nothing else, it is erased unless if this is a
// CallInst, in which case it is tracked for future use
...
bool get_operand_repr(llvm::Value *operand, CFG_Opd **repr, const GlobalInfo &globals) {
  if (llvm::Instruction *inst = llvm::dyn_cast<llvm::Instruction>(operand)) {
    ...
    *repr = new CFG_Opd(CFG_OpdType::CFG_NumOpd,
                        std::atoi(std::string(vec.begin(), vec.end()).c_str()));
    return true;
  }
...
RHS get_assignment_value(llvm::Value *value, const GlobalInfo &globals,
  ...
  static const std::map<unsigned int, char> ops{{llvm::Instruction::Add, '+'},
...
// Returns a list of CFG nodes and their corresponding llvm values for a llvm
// basic block
std::vector<std::pair<CFG_Node *, llvm::Value *>>
get_nodes_in_basic_block(const std::string &proc, llvm::BasicBlock *bb, const GlobalInfo &globals) {
  std::vector<std::pair<CFG_Node *, llvm::Value *>> res;
  std::set<llvm::Value *> removable_uses;

  if (bb->isEntryBlock()) {
    res.push_back(
        std::make_pair(new CFG_Node(CFG_NodeType::CFG_StartNode, 0, "START " + proc), nullptr));

    // Add in dummy assignments to indicate what the initial value of these
    // variables is
    if (proc == "main") {
      for (llvm::GlobalVariable *var : globals.globals) {
        CFG_Opd *ropd1;
        if (get_operand_repr(var->getOperand(0), &ropd1, globals)) {
          res.push_back(std::make_pair(
              new CFG_Node(CFG_NodeType::CFG_AssignNode, 0, "=",
                           new CFG_Opd(CFG_OpdType::CFG_VarOpd, var->getName().str()), ropd1,
                           nullptr),
              nullptr));
        }
      }
    ...
    if (it != globals.uses.end()) {
      ...
      if (llvm::CallInst *call = llvm::dyn_cast<llvm::CallInst>(&inst)) {
        ...
        if (it != globals.functions.end()) {
          res.push_back(std::make_pair(
              new CFG_Node(CFG_NodeType::CFG_CallNode, 0, "CALL " + *it, *it), call));
          make_usevar = call->getNumUses() > 0;
        }
      }
      if (make_usevar) {
        // If this is actually not used by a USEVAR, this will be marked as
        // erasible and removed later
        res.push_back(
            std::make_pair(new CFG_Node(CFG_NodeType::CFG_AssignNode, 0, "=",
                                        new CFG_Opd(CFG_OpdType::CFG_UsevarOpd),
                                        new CFG_Opd(CFG_OpdType::CFG_VarOpd, it->second), nullptr),
                           &inst));
      }
    } else {
      ...
      if (it != globals.defs.end()) {
        if (llvm::StoreInst *store = llvm::dyn_cast<llvm::StoreInst>(&inst)) {
          ...
          res.push_back(std::make_pair(
              new CFG_Node(CFG_NodeType::CFG_AssignNode, 0, rhs.op,
                           new CFG_Opd(CFG_OpdType::CFG_VarOpd, it->second), rhs.ropd1, rhs.ropd2),
              store));
          ...
          if (ret->getNumOperands() == 1) {
            ...
            res.push_back(
                std::make_pair(new CFG_Node(CFG_NodeType::CFG_AssignNode, 0, rhs.op,
                                            new CFG_Opd(CFG_OpdType::CFG_VarOpd, it->second),
                                            rhs.ropd1, rhs.ropd2),
                               ret));
          }
        }
  ...
  for (auto pair : res) {
    ...
    if (pair.first->get_type() != CFG_NodeType::CFG_AssignNode ||
        removable_uses.find(pair.second) == removable_uses.end()) {
...
// Given a map of globals and the number of interactions they are in, returns
// the global in the most interactions
...
// Creates a partition from globals that have not yet been partitioned, and
// unions all of these globals into the same set in partitions
void create_partition(std::map<std::string, int> &globals,
  ...
  // Keep adding to the partition until the max partition size is reached or no
  // more globals are reachible from first Globals are added in the order of
  // most interations with the rest of the partition
...
// Creates disjoint sets representing the global partitions given a list of
// SimpleProcs
...
// Given a CFG node and llvm value, split the node into smaller nodes (involving
// "USEVAR" and "INPUT") if it involves variables from different partitions
// Return the resulting CFG node - llvm value pairs
std::vector<std::pair<CFG_Node *, llvm::Value *>> split_node(CFG_Node *node, llvm::Value *value,
                                                             const DisjointSets &partitions) {
  ...
  if (lopd->get_type() == CFG_OpdType::CFG_VarOpd) {
      ...
      if (!partitions.are_eq(def, use)) {
        ...
        for (size_t i = 0; i < ropds.size(); ++i) {
          if (ropds[i]->get_type() == CFG_OpdType::CFG_VarOpd) {
            res.push_back(std::make_pair(
                new CFG_Node(
                    CFG_NodeType::CFG_AssignNode, 0, "=", new CFG_Opd(CFG_OpdType::CFG_UsevarOpd),
                    new CFG_Opd(CFG_OpdType::CFG_VarOpd, ropds[i]->get_opd_var()), nullptr),
                ropds.size() == 1 ? value
                                  : llvm::dyn_cast<llvm::Instruction>(
                                        llvm::dyn_cast<llvm::StoreInst>(value)->getValueOperand())
                                        ->getOperand(i)));
          }
        }

        // Add an "lopd = INPUT" node
        res.push_back(std::make_pair(new CFG_Node(CFG_NodeType::CFG_AssignNode, 0, "=",
                                                  new CFG_Opd(CFG_OpdType::CFG_VarOpd, def),
    ...
    if (value != nullptr) {
      // If the node was not split and this node maps to a llvm value, erase the
      // llvm value if it can be recovered from the CFG node
...
void convert_to_cfg(SimpleProc &simple_proc, const DisjointSets &partitions) {
  ...
  // Add the end node
  CFG_Node *end_node =
      new CFG_Node(CFG_NodeType::CFG_EndNode, node_num, "END " + simple_proc.proc_name);
