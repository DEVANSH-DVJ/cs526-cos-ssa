...
// Converts a cfg_opd to a ssa_opd, accounting for whether the cfg_opd is part of the current partition (if it is a variable)
// Final versions maps variables to a known ssa_opd that should be used
...
void ssa_construct_partition() {
  for (QDef qdef : program->get_ddg_nodes()) {
    ...
    for (auto& [use, meta_nums] : versions) {
      ...
      stmts->push_back(new SSA_Stmt(SSA_PhiStmt, new SSA_Opd(SSA_PhiOpd, std::make_pair(qdef.def.node, qdef.context), use), ropds));
      final_versions[use] = new SSA_Opd(SSA_PhiOpd, std::make_pair(qdef.def.node, qdef.context), use);
    }
